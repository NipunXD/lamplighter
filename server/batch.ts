// Headless batch runner: pnpm batch --seed 7 --size 120 --llm on --razorpay off --days 7 --chaos 0
...
const cfg = { seed: Number(args.seed ?? 7), size: Number(args.size ?? 120), llm: on(args.llm, true), razorpay: on(args.razorpay, false), simDays: Number(args.days ?? 7), chaos: Number(args.chaos ?? 0), mode: 'agent' as const, tickDelayMs: 0 };
