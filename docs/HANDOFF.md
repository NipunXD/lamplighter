...
- `web/src/review/` — the Week-in-Review analysis panel was being built by an agent (charts over `snapshot.timeline`, money flow, root causes, agent vs cron). `review.css` and `preview.tsx` exist; `WeekInReview.tsx` had not landed when the session ended. Until it exists, `pnpm typecheck` fails on `web/src/review/preview.tsx` — either finish the component (brief in `docs/REVIEW_BRIEF.md`) or delete the folder. `web/src/mock.ts` has an uncommitted edit from the same agent (emits timelines); it typechecks.
- The ledger already has a "Week in review →" button prop (`onReview`); wire it in `web/src/App.tsx` by mounting `<WeekInReview snapshot={snapshot} onClose live={phase==='running'} />` inside `.town-panel` in a `div.review-host`, and import `./review/review.css`.
...
1. Create the public repo and push (GitHub CLI is logged in as NipunXD):
   ```bash
   gh repo create lamplighter --public --source . --remote origin --push --description "Bounded AI revenue-recovery agent for Razorpay merchants, rendered as a cozy town"
   ```
2. Record the 5-minute video with `docs/PITCH.md` (QuickTime: File → New Screen Recording). Upload unlisted to YouTube or Loom.
